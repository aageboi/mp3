...
<channel>
	...
	<description><![CDATA[Kumpulan Dzikir dan Doa sesuai sunnah]]></description>
	<itunes:owner>
		<itunes:name>Heri Gunawan</itunes:name>
  ...
  foreach ($files as $key => $file) {
    ...
    $mp3file = new MP3File($file['filename']);
    $duration = $mp3file->getDurationEstimate();

    $date = date('D, d M Y H:i:s');
    $name = str_replace('&', '&amp;', $file['title']);
    $desc = str_replace('&', '&amp;', $file['description']);
    $fileurl = $file['url'];
    $author = $file['author'];

    $text .= "\t<item>". PHP_EOL .
    "\t\t<itunes:title>{$name}</itunes:title>". PHP_EOL .
    "\t\t<title>{$name}</title>". PHP_EOL .
    "\t\t<description><![CDATA[<p>{$desc}</p>]]></description>". PHP_EOL .
    "\t\t<content:encoded><![CDATA[<p>{$desc}</p>]]></content:encoded>". PHP_EOL .
    "\t\t<itunes:author>{$author}</itunes:author>". PHP_EOL .
    "\t\t<enclosure url=\"{$fileurl}\" length=\"{$filesize}\" type=\"audio/mpeg\" />". PHP_EOL .
    "\t\t<guid isPermaLink=\"false\">dzikir-doa-{$key}</guid>". PHP_EOL .
    "\t\t<pubDate>{$date} +0700</pubDate>". PHP_EOL .
    "\t\t<itunes:duration>{$duration}</itunes:duration>". PHP_EOL .
    "\t\t<itunes:keywords></itunes:keywords>". PHP_EOL .
    "\t\t<itunes:season>1</itunes:season>". PHP_EOL .
    "\t\t<itunes:episode>{$key}</itunes:episode>". PHP_EOL .
    "\t\t<itunes:episodeType>full</itunes:episodeType>". PHP_EOL .
    "\t\t<itunes:explicit>false</itunes:explicit>". PHP_EOL .
    "\t</item>" . PHP_EOL;
  }
